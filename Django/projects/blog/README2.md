...
N.b. json loads as a dictionary
  
scroll down to the email_host_user section at bottom of the file
  
N.b. we previouly used environment variables we are now using config so change environ.get to config.get for both 
...
N.b. you could create a test server with debug set to true to test the server
